...
type GophProfileRepository interface {
	...
	UploadAvatar(ctx context.Context, userID string, fileName string, mimeType string, sizeBytes int64, s3Key string, thumbnailS3Keys string, uploadStatus string, processingStatus string) (*model.Avatar, error) // загружает аватарку
	...
	GetAvatarMetadataByID(ctx context.Context, avatarID string) (*model.AvatarMetadata, error)                                                                                                                     // получает метаданные аватарки по ID
	GetAvatarMetadataByUserID(ctx context.Context, userID string) (*model.AvatarMetadata, error)                                                                                                                   // получает метаданные аватарки по ID пользователя
